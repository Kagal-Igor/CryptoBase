...
namespace CryptoBase
{
    public class MD5 : iHash
    {
        ...
        protected static byte[] m_byteInput;

        public void GetHash(Stream input, Stream output)
        {
            m_byteInput = new byte[input.Length];
            input.Position = 0;
            int size = input.Read(m_byteInput, 0, (int)input.Length);

            string hash = CalculateMD5Value().ToString();

            output.Write(Encoding.Default.GetBytes(hash), 0, Encoding.Default.GetBytes(hash).Length);
        }
        ...
        protected void Transform(ref uint A, ref uint B, ref uint C, ref uint D)
        {
            uint AA, BB, CC, DD;
